...
(function () {
  ...
  const SUMMARY_PROMPT = `请用简体中文分析下方网页内容，专有名词保留原文。直接输出结果，排版紧凑。
只分析与网页标题直接相关的主体内容，忽略备案信息、版权声明、导航菜单、广告、推荐列表等无关内容。
所有链接必须使用完整 Markdown 格式 [文本](URL)，确保可点击。
...
用一小段文字概括要点，然后用 Markdown 表格结构化呈现关键信息。
表头根据内容自适应选择（如 | 要点 | 详情 | 或 | 维度 | 内容 |）。
保留原文中的数字、日期、人名、产品名等事实，重点内容加粗。
